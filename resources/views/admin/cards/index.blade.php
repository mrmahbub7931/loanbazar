...
<div class="card">
    ...
    <div class="ml-auto m-3">
        <a href="{{ route('admin.cards.create') }}" class="btn btn-sm btn-outline-success px-5 radius-30">Create Card Service</a>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table id="applicationTable" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $card->service_title }}</td>
                        <td>{{ date('F d, Y', strtotime($card->created_at)) }}</td>
                        @if($card->status == '1')
                        ...
                        <td>
							...
							<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end">	
                                <a class="dropdown-item" href="{{ route('admin.cards.edit', $card->id) }}">Edit</a>
								<a onclick="deleteCards({{ $card->id }})" class="dropdown-item" href="javascript:;">Delete</a>
                                <form action="{{ route('admin.cards.delete', $card->id) }}" method="post"
                                    style="display: none" id="delete-form-{{ $card->id }}">
                                    @csrf
                                    @method('DELETE')
                                </form>
							</div>
