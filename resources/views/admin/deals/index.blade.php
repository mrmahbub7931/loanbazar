            ...
            <table id="sliderTable" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $deal->title }}</td>
                        <td>{{ getUsername($deal->user_id) }}</td>
                        <td>{{ date('F d, Y', strtotime($deal->created_at)) }}</td>
                        @if($deal->status == 1)
                        ...
                        <td>
							...
							<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end">	
                                <a class="dropdown-item" href="{{ route('admin.deals.edit',$deal->id) }}">Edit</a>
								<a onclick="deleteDeals({{$deal->id}})" class="dropdown-item" href="javascript:;">Delete</a>
                                <form action="{{ route('admin.deals.delete',$deal->id) }}" method="post"
                                    style="display: none" id="delete-form-{{$deal->id}}">
                                    @csrf
                                    @method('DELETE')
                                </form>
							</div>
