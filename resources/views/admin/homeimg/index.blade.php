...
<div class="card">
    ...
    <div class="ml-auto m-3">
        <a href="{{ route('admin.homeimg.create') }}" class="btn btn-sm btn-outline-success px-5 radius-30">New Image</a>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table id="reviewsTable" class="table table-striped table-bordered">
                ...
                <tbody>
                    {{-- {{ $homeimages }} --}}
                    @foreach ($homeimages as $key => $image)
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $image->image_title }}</td>
                        <td>{{ getUsername($image->user_id) }}</td>
                        ...
                        <td>
							...
							<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end">	
                                <a class="dropdown-item" href="{{ route('admin.homeimg.edit',$image->id) }}">Edit</a>
								<a onclick="deleteHomeimage({{$image->id}})" class="dropdown-item" href="javascript:;">Delete</a>
                                <form action="{{ route('admin.homeimg.delete',$image->id) }}" method="post"
