...
@section('title', 'Loan Docs Update')
        ...
        @endif
            <div class="card-body p-5">
                <div class="card-title d-flex align-items-center">
                    ...
                    <h5 class="mb-0 text-primary">Update Loan Docs</h5>
                </div>
                <hr>
                <form class="row g-3" action="{{ route('admin.loans.docs.update', $loanDoc->id) }}" id="serviceDocsForm" method="post">
                    @csrf
                    @method('PUT')
                    <div class="col-md-12">
                        <label for="service_title" class="form-label">Documents Title</label>
						<input class="form-control" type="text" id="service_title" name="service_title" value="{{ $loanDoc->title }}">
                    </div>
                    
                    <div class="col-md-12">
                        <label for="serviceDocs" class="form-label">Service Docs</label>
						<textarea name="service_docs" id="serviceDocs" cols="30" rows="10">{{ $loanDoc->body }}</textarea>
                    </div>
                    <div class="col-md-6 col-12">
                        ...
                        <select name="loan_service_id" id="loan_service_id" class="form-control">
                            ...
                            <option value="{{ $item->id }}" {{ $loanDoc->loan_service_id == $item->id ? 'selected' : '' }}>{{ $item->service_title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 col-12">
                        ...
                        <select name="status" id="status" class="form-control @error( 'status' ) is-invalid @enderror">
                            <option value="1" {{$loanDoc->status == '1' ? 'selected' : ''}}>Active</option>
                            <option value="0" {{$loanDoc->status == '0' ? 'selected' : ''}}>Inactive</option>
                        </select>
                    </div>
                    <div class="col-12">
