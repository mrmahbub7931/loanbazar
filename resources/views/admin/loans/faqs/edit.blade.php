        ...
        @endif
            <div class="card-body p-5">
                <div class="card-title d-flex align-items-center">
                    ...
                    <h5 class="mb-0 text-primary">Update Card Faq</h5>
                </div>
                <hr>
                <form class="row g-3" action="{{ route('admin.cards.faqs.update', $cardFaq->id) }}" id="applicationForm" method="post">
                    @csrf
                    @method('PUT')
                    <div class="col-md-12">
                        <label for="faq_title" class="form-label">Faq Title</label>
						<input class="form-control" type="text" id="faq_title" name="faq_title" value="{{ $cardFaq->faq_title }}">
                    </div>
                    
                    <div class="col-md-12">
                        <label for="faq_description" class="form-label">Faq Description</label>
						<textarea name="faq_description" id="faq_description" cols="30" rows="10">{{ $cardFaq->faq_description }}</textarea>
                    </div>
                    <div class="col-md-6 col-12">
                        ...
                        <select name="card_service_id" id="card_service_id" class="form-control">
                            ...
                            <option value="{{ $item->id }}" {{ $cardFaq->card_service_id == $item->id ? 'selected' : '' }}>{{ $item->service_title }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6 col-12">
