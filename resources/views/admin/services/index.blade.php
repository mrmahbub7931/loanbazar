...
<div class="card">
    ...
    <div class="ml-auto m-3">
        <a href="{{ route('admin.services.create') }}" class="btn btn-sm btn-outline-success px-5 radius-30">Create Services</a>
    </div>
    <div class="card-body">
            ...
            <table id="sliderTable" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $key + 1 }}</td>
                        <td>{{ $service->title }}</td>
                        <td>{{ getUsername($service->user_id) }}</td>
                        ...
                        <td>
							...
							<div class="dropdown-menu dropdown-menu-right dropdown-menu-lg-end">	
								...
								<a onclick="deleteservices({{$service->id}})" class="dropdown-item" href="javascript:;">Delete</a>
                                <form action="{{ route('admin.services.delete',$service->id) }}" method="post"
                                    style="display: none" id="delete-form-{{$service->id}}">
                                    @csrf
                                    @method('DELETE')
                                </form>
							</div>
